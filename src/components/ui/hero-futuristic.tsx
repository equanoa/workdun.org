...
const PostProcessing = ({
  ...
  const render = useMemo(() => {
    ...
    postProcessing.outputNode = final;
      ...
      console.error('PostProcessing initialization error:', error);
      return null;
    }
...
const RendererInitializer = ({ onReady }: { onReady: () => void }) => {
  ...
  useEffect(() => {
    if (gl && 'init' in gl && typeof (gl as any).init === 'function') {
      (gl as any).init().then(() => {
        onReady();
      }).catch((error: any) => {
...
export const Html = () => {
  const titleWords = 'Workdun'.split(' ');
  const subtitle = 'Scale. Grow. Dominate.';
  ...
  return (
    <div className="h-svh relative rounded-b-[3rem] overflow-hidden border-b-4 border-white" style={{ backgroundColor: 'white' }}>
      <div className="absolute inset-0 bg-background rounded-b-[3rem]" style={{ backgroundColor: 'hsl(0, 0%, 3%)' }}></div>
      ...
      <div className="absolute inset-0 rounded-b-[3rem] overflow-hidden" style={{ backgroundColor: 'white' }}>
        <Canvas
          flat
          gl={(canvas) => {
